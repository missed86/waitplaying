...
const Component = styled.div`
    ...
    background-color: ${(props) => props.service && services[props.service].color};
    padding: 5px 5px;
    border-radius: 5px;
...
export default function FilterCheckbox({ service, filter, setFilter }) {
	...
	return (
		<Component service={service} onClick={handleClick} active={filter[service]}>
			<Icon active={filter[service]}>{services[service].icon}</Icon>
			<Label active={filter[service]}>{services[service].label}</Label>
		</Component>
	);
}
