...
export default function OnServicesTable({ services }) {
	...
	const dateLabel = (service) => {
		if (services[service].active) {
			return (
				<>
					{"Since"} <span>{formattedDate(services[service].start_date)}</span>
				</>
			);
		} else {
			return (
				<>
					{"Until"} <span>{formattedDate(services[service].end_date)}</span>
				</>
			);
		}
	};
